...
public final class AlchemySetup {
	...
	public static void registerCapabilities(RegisterCapabilitiesEvent event) {
		...
		event.registerBlockEntity(Capabilities.Item.BLOCK, ModBlockEntities.BREWING_BARREL.get(),
				(barrel, side) -> barrel.getAutomationHandler());
		event.registerBlockEntity(Capabilities.Item.BLOCK, ModBlockEntities.CRUSHING_TUB.get(), (tub, side) -> tub.getItems());
		event.registerBlockEntity(Capabilities.Fluid.BLOCK, ModBlockEntities.CRUSHING_TUB.get(), (tub, side) -> tub.getFluidHandler());
		event.registerBlockEntity(Capabilities.Item.BLOCK, ModBlockEntities.EVAPORATING_BASIN.get(),
				(basin, side) -> basin.getAutomationHandler());
		event.registerBlockEntity(Capabilities.Fluid.BLOCK, ModBlockEntities.EVAPORATING_BASIN.get(), (basin, side) -> basin.getTank());
		event.registerBlockEntity(Capabilities.Fluid.BLOCK, ModBlockEntities.LIQUID_BARREL.get(), (barrel, side) -> barrel.getTank());

		event.registerItem(Capabilities.Fluid.ITEM, (stack, access) -> new FluidBottleItem.FluidHandler(access), ModItems.FLUID_BOTTLE.get());
		event.registerItem(Capabilities.Fluid.ITEM, (stack, access) -> new LiquidBarrelItem.FluidHandler(access),
	...
	private static void onUseGlassBottleOnBlock(PlayerInteractEvent.RightClickBlock event) {
		...
		ResourceHandler<FluidResource> tank = level.getCapability(Capabilities.Fluid.BLOCK, event.getPos(), event.getFace());
		if (tank == null) return;
		FluidResource resource = ResourceHandlerUtil.findExtractableResource(tank, fluid -> true, null);
		if (resource == null || extractBottle(tank, resource, false) < FluidBottleItem.CAPACITY) return;
		FluidStack available = resource.toStack(FluidBottleItem.CAPACITY);
		ItemStack filled;
		...
		if (!level.isClientSide()) {
			extractBottle(tank, resource, true);
	...
	/** Takes (or simulates taking) one bottle of the fluid out of the tank; returns the amount taken. */
	private static int extractBottle(ResourceHandler<FluidResource> tank, FluidResource resource, boolean execute) {
		try (Transaction tx = Transaction.openRoot()) {
			int extracted = tank.extract(resource, FluidBottleItem.CAPACITY, tx);
			if (execute && extracted == FluidBottleItem.CAPACITY) {
				tx.commit();
			}
			return extracted;
		}
	...
	private static void onAttackEntity(AttackEntityEvent event) {
		...
		if (appliedDuration > 0) {
			if (instant) {
				effect.value().applyInstantenousEffect(serverLevel, player, player, target, amplifier, 1.0D);
			} else {
