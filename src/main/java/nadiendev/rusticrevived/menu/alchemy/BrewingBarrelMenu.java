...
public class BrewingBarrelMenu extends AbstractContainerMenu {
	...
	public BrewingBarrelMenu(int containerId, Inventory playerInventory, BrewingBarrelBlockEntity barrel, ContainerData data) {
		...
		this.barrel = barrel;
		this.data = data;
		IItemHandler items = barrel.getItems();
	...
	private boolean moveToBarrel(ItemStack stack) {
		...
		FluidStack fluid = stack.is(Items.GLASS_BOTTLE) ? FluidStack.EMPTY : FluidUtil.getFluidContained(stack).orElse(FluidStack.EMPTY);
		if (fluid.isEmpty()) {
	...
	private static final class ContainerSlot extends SlotItemHandler {
		ContainerSlot(IItemHandler items, int index, int x, int y) {
