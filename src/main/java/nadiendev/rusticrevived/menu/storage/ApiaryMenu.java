...
public class ApiaryMenu extends AbstractContainerMenu {
	...
	public ApiaryMenu(int containerId, Inventory playerInventory, IItemHandler bees, IItemHandler honeycomb, ContainerLevelAccess access) {
		super(ModMenus.APIARY.get(), containerId);
		this.access = access;
		addSlot(new SlotItemHandler(bees, 0, 80, 26) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return stack.is(ModItems.BEE.get());
			}
		});
		addSlot(new SlotItemHandler(honeycomb, 0, 80, 44) {
			@Override
			public boolean mayPlace(ItemStack stack) {
