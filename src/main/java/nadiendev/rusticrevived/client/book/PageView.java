...
public interface PageView {
	...
	void render(AlmanacScreen screen, GuiGraphics graphics, int mouseX, int mouseY, float partialTick);
	...
	record Categories(CategoriesPage page) implements PageView {
		...
		@Override
		public void render(AlmanacScreen screen, GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
		...
		public void render(AlmanacScreen screen, GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
			...
			for (String key : page.getTextKeys()) {
				for (FormattedCharSequence line : font.split(Component.translatable(key), (int) ((AlmanacScreen.WIDTH - 32) / TEXT_SCALE))) {
					...
					graphics.drawString(font, line, (int) (x / TEXT_SCALE), (int) (y / TEXT_SCALE), 0x000000, false);
					y += increment;
				}
				y += (int) (4 * TEXT_SCALE);
			}
			...
			if (!page.getRelatedEntries().isEmpty()) {
				...
				graphics.blit(AlmanacScreen.BOOK_TEXTURE, left + AlmanacScreen.WIDTH, top + 10, (146 - 10) - (labelWidth + 2), 210, labelWidth + 2 + 10, 16);
				graphics.drawString(font, label, left + AlmanacScreen.WIDTH + 2, top + 10 + 4, 0x000000, false);
			}
		}
	}
}
