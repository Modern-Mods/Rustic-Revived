Crushing Tubs can be made with any wooden planks, any wooden slabs, and iron. They are used to crush solids into liquids. They can hold up to one stack of items, and up to eight buckets of liquid.  
All recipes should be visible in JEI.  
[KubeJS](KubeJS-Support.md) support is available so that modpack makers can add and remove recipes.

![](http://i.imgur.com/ArHWhtF.png)

***

## Usage ##
To use a crushing tub, place it in the world. Then, insert items into it by right-clicking the block with items in hand. Remove items by right-clicking with an empty hand. Once a tub has items inside, if the items are a valid recipe, jumping on the tub will crush the items into their corresponding fluid, one at a time. If the tub is too full of fluid, crushing will not occur. You will be alerted when crushing is successful with a squishing sound, as well as the appearance of fluid in the tub, and the disappearance of items in the tub. If necessary, you can void all fluid contained in the tub by shift-right-clicking the block with an empty hand.  
Some crushing tub recipes produce both fluid and items. If a recipe has an item byproduct, then the item will appear as an entity in the world above the tub when crushing occurs.

## Recipes ##
+ [Olives](Olives.md) -> [Olive Oil](Olives.md)
+ [Ironberries](Ironberries.md) -> [Ironberry Juice](Ironberries.md)
+ Sugar Canes -> Water + 2 Sugar
+ [Wildberries](Wildberries.md) -> [Wildberry Juice](Wildberries.md)
+ [Grapes](Grapes.md) -> [Grape Juice](Grapes.md)
+ Apples -> Apple Juice + [Apple Seeds](Apple-Trees.md)
+ [Honeycomb](Bees.md) -> [Honey](Bees.md)
+ Golden Apples -> Golden Apple Juice + [Apple Seeds](Apple-Trees.md)
+ Enchanted Golden Apples -> 1 bucket of Golden Apple Juice + [Apple Seeds](Apple-Trees.md)
+ Vanta Lilies -> [Vanta Oil](Vanta-Oil.md)