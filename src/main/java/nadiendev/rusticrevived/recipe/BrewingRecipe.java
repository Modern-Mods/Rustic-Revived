 ...
 * </pre>
 */
public record BrewingRecipe(FluidIngredient input, FluidStackTemplate result, QualityMode quality) implements RusticMachineRecipe<FluidRecipeInput> {
	...
	public static final MapCodec<BrewingRecipe> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
			...
			FluidStackTemplate.CODEC.fieldOf("result").forGetter(BrewingRecipe::result),
	...
	public boolean matchesCulture(FluidStack culture) {
		return culture.isEmpty() || culture.getFluid() == result.fluid().value();
	}
	...
	public FluidStack brew(@Nullable FluidStack culture, int amount, RandomSource rand) {
		FluidStack out = result.create().copyWithAmount(amount);
	...
	/** The resulting booze, without quality. */
