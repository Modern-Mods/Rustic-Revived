...
public class AlchemyCategory extends TexturedRecipeCategory<RecipeHolder<CondenserRecipe>> {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<CondenserRecipe> holder, IFocusGroup focuses) {
		...
		long amount = recipe.fluid().amount();
		...
		builder.addSlot(RecipeIngredientRole.OUTPUT, 82, 32).addItemStack(recipe.result());
	}

	@Override
	public void draw(RecipeHolder<CondenserRecipe> holder, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		super.draw(holder, recipeSlotsView, guiGraphics, mouseX, mouseY);
		int time = holder.value().getTime();
		flame.draw(guiGraphics, 44, 43);
		arrows.computeIfAbsent(time, ticks -> guiHelper.createAnimatedDrawable(arrowSprite, ticks, IDrawableAnimated.StartDirection.LEFT, false))
				.draw(guiGraphics, 21, advanced ? 14 : 26);
		drawRightAligned(guiGraphics, duration(time), 128, 68);
	}
}
