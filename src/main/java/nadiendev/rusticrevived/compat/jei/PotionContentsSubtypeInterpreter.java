...
public final class PotionContentsSubtypeInterpreter implements ISubtypeInterpreter<ItemStack> {
	...
	public String getLegacyStringSubtypeInfo(ItemStack stack, UidContext context) {
		...
		StringJoiner joiner = new StringJoiner(",", contents.potion().map(potion -> potion.getRegisteredName()).orElse("") + ";", "");
		for (MobEffectInstance effect : contents.customEffects()) {
			joiner.add(effect.getEffect().getRegisteredName() + ":" + effect.getAmplifier() + ":" + effect.getDuration());
		}
