...
public class EvaporatingCategory extends TexturedRecipeCategory<RecipeHolder<EvaporatingBasinRecipe>> {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<EvaporatingBasinRecipe> holder, IFocusGroup focuses) {
		...
		builder.addSlot(RecipeIngredientRole.INPUT, 21, 16)
				...
				.addIngredients(NeoForgeTypes.FLUID_STACK, fluidStacks(recipe.fluid().ingredient(), recipe.getAmount()));
		builder.addSlot(RecipeIngredientRole.OUTPUT, 71, 24).addItemStack(recipe.getResultStack());
	}

	@Override
	public void draw(RecipeHolder<EvaporatingBasinRecipe> holder, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
		super.draw(holder, recipeSlotsView, graphics, mouseX, mouseY);
		drawCentered(graphics, duration(holder.value().getTime()), 54, 54);
	}
}
