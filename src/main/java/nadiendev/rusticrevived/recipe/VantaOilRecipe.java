...
public class VantaOilRecipe extends CustomRecipe {
	...
	public static final VantaOilRecipe INSTANCE = new VantaOilRecipe();

	private VantaOilRecipe() {
	}
	...
	/** Axes, the {@code #minecraft:swords} and {@code #minecraft:axes} tags, the {@link ModTags.Items#VANTA_OILABLE} tag and the config whitelist ({@code id} or {@code #tag}). */
	public static boolean isOilable(ItemStack stack) {
		if (stack.getItem() instanceof AxeItem || stack.is(ItemTags.SWORDS) || stack.is(ItemTags.AXES) || stack.is(ModTags.Items.VANTA_OILABLE)) {
			return true;
		}
	...
	@Override
	public RecipeSerializer<VantaOilRecipe> getSerializer() {
