...
public abstract class AbstractCondenserMenu extends AbstractContainerMenu {
	...
	protected AbstractCondenserMenu(MenuType<?> type, int containerId, Inventory playerInventory, AbstractCondenserBlockEntity condenser,
		...
		this.data = data;
		IItemHandler items = condenser.getItems();
		addSlot(new SlotItemHandler(items, AbstractCondenserBlockEntity.SLOT_RESULT, 105, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
		});
		addSlot(new SlotItemHandler(items, AbstractCondenserBlockEntity.SLOT_FUEL, 66, 62) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return AbstractCondenserBlockEntity.isFuel(stack);
			}
		});
		addSlot(new SlotItemHandler(items, AbstractCondenserBlockEntity.SLOT_BOTTLE, 105, 7));
	...
	protected abstract void addIngredientSlots(IItemHandler items);
	...
	private boolean moveToMachine(ItemStack stack) {
		...
		if (AbstractCondenserBlockEntity.isFuel(stack)) {
