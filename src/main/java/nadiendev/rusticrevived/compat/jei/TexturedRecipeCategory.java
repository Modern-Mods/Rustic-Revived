 ...
 * sheet (256x256) as background and provides small text and fluid helpers.
 */
public abstract class TexturedRecipeCategory<T> extends AbstractRecipeCategory<T> {
	...
	protected TexturedRecipeCategory(IGuiHelper guiHelper, IRecipeType<T> type, String titleKey, ItemLike icon, String textureName,
	...
	protected static void drawCentered(GuiGraphicsExtractor graphics, Component text, int centerX, int y) {
		Font font = Minecraft.getInstance().font;
		graphics.text(font, text, centerX - font.width(text) / 2, y, TEXT_COLOR, false);
	}

	protected static void drawRightAligned(GuiGraphicsExtractor graphics, Component text, int right, int y) {
		Font font = Minecraft.getInstance().font;
		graphics.text(font, text, right - font.width(text), y, TEXT_COLOR, false);
	}
	...
	/** One stack of the given amount per fluid accepted by the ingredient. */
	protected static List<FluidStack> fluidStacks(FluidIngredient ingredient, int amount) {
		return ingredient.fluids().stream().map(fluid -> new FluidStack(fluid, amount)).toList();
	}
}
