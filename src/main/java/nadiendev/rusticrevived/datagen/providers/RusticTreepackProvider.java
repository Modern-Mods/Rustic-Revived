...
public class RusticTreepackProvider implements DataProvider {
	...
	public CompletableFuture<?> run(CachedOutput cache) {
		...
		writes.add(save(cache, "resourcepacks/dynamictrees/pack.mcmeta", """
				{"pack": {"description": "Rustic Revived: Dynamic Trees assets", "pack_format": 34}}"""));
	...
	private void assets(CachedOutput cache, List<CompletableFuture<?>> writes, Tree tree) {
		...
		writes.add(save(cache, assets + "blockstates/" + name + "_branch.json", variant(NS + ":block/" + name + "_branch")));
		writes.add(save(cache, assets + "models/block/" + name + "_branch.json", "{\"loader\": \"dynamictrees:branch\", " + bark + "}"));
	...
	private void lootTables(CachedOutput cache, List<CompletableFuture<?>> writes, Tree tree) {
		...
		writes.add(save(cache, loot + "trees/branches/" + name + ".json", """
				{"neoforge:conditions": [{"type": "neoforge:mod_loaded", "modid": "dynamictrees"}], "type": "minecraft:block", "random_sequence": "%1$s:trees/branches/%2$s", "pools": [
