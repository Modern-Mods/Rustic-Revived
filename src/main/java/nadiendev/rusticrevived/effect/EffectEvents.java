...
public final class EffectEvents {
	...
	/** Inebriation of entities currently drinking milk, restored once the milk cleared every effect. */
	private static final Map<LivingEntity, MobEffectInstance> TIPSY_WHILE_DRINKING_MILK = new WeakHashMap<>();

	private EffectEvents() {
	}

	/** Milk clears every effect since 1.21.2; being drunk is not one of the things it cures. */
	@SubscribeEvent
	public static void onMilkTick(LivingEntityUseItemEvent.Tick event) {
		LivingEntity entity = event.getEntity();
		if (entity.level().isClientSide() || !event.getItem().is(Items.MILK_BUCKET)) return;
		MobEffectInstance tipsy = entity.getEffect(ModEffects.TIPSY);
		if (tipsy != null) {
			TIPSY_WHILE_DRINKING_MILK.put(entity, new MobEffectInstance(tipsy));
		}
	}

	@SubscribeEvent
	public static void onMilkFinish(LivingEntityUseItemEvent.Finish event) {
		MobEffectInstance tipsy = TIPSY_WHILE_DRINKING_MILK.remove(event.getEntity());
		if (tipsy != null && event.getItem().is(Items.MILK_BUCKET) && !event.getEntity().hasEffect(ModEffects.TIPSY)) {
			event.getEntity().addEffect(tipsy);
		}
	}
